...
def RandomForest(x_train, x_test, y_train, y_test, seed):
    ...
    save_path = os.path.join(project_root, 'results', 'Random Forest', 'cm_rf.png')
    ...
    feature_selection_methods = {
        "Chi-Squared": SelectKBest(score_func=chi2, k=5),  # Imposta k al numero desiderato di feature
        "Forward Selection": SequentialFeatureSelector(RandomForestClassifier(), n_features_to_select=5, direction='forward'),
        "Backward Selection": SequentialFeatureSelector(RandomForestClassifier(), n_features_to_select=5, direction='backward'),
        "Exhaustive Search": SequentialFeatureSelector(RandomForestClassifier(), n_features_to_select=5),
    ...
    for method, selector in feature_selection_methods.items():
        ...
        # Addestra un classificatore (sostituisci con il classificatore desiderato)
        RandomForest = RandomForestClassifier()  # Sostituisci con il classificatore desiderato
    ...
    shap_path = os.path.join(project_root, 'results', 'Random Forest', 'shap_rf.png')
